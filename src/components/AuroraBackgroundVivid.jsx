...
export default function AuroraBackgroundVivid({
  ...
  const makeNoise = () => {
    ...
    return (x, y) => {
      ...
      const n0 = grad(aa, x, y), n1 = grad(ba, x - 1, y), n2 = grad(ab, x, y - 1), n3 = grad(bb, x - 1, y - 1);
      const ix0 = lerp(n0, n1, u), ix1 = lerp(n2, n3, u);
  ...
  const buildSky = (w, h, dpr) => {
    ...
    if (isLight) {
      ...
      if (lightSky === "white") { ctx.fillStyle = "#fff"; ctx.fillRect(0, 0, w, h); return c; }
      const g = ctx.createLinearGradient(0, 0, 0, h);
      g.addColorStop(0, "#eaf4ff"); g.addColorStop(1, "#eef6ff");
      ctx.fillStyle = g; ctx.fillRect(0, 0, w, h);
      return c;
    }

    const g = ctx.createLinearGradient(0, 0, 0, h);
    g.addColorStop(0, "#07132b"); g.addColorStop(0.5, "#0b1a38"); g.addColorStop(1, "#12244c");
    ctx.fillStyle = g; ctx.fillRect(0, 0, w, h);
    return c;
  };
  ...
  useEffect(() => {
    ...
    const render = (now) => {
      ...
      if (starsCache.current) {
        const tw = starTwinkle ? (0.92 + 0.05 * Math.sin(now * 0.0011) + 0.03 * Math.cos(now * 0.002)) : 1;
      ...
      if (allowAurora) {
        ...
        const baseAlpha = eff * (isLight ? 1.20 : 1.0); // boost light mode

        // leader + followers (more separation + phase)
        const threeSpec = [
          { role: "center", sign: 0,  phase: 0.00, alpha: 1.00, sep: 0.00 },
          { role: "upper",  sign: -1, phase: +0.45, alpha: 0.88, sep: 0.20 },
          { role: "lower",  sign: +1, phase: -0.45, alpha: 0.88, sep: 0.20 },
        ];
        const specs = ribbonCount === 3
          ? threeSpec
          : Array.from({ length: ribbonCount }, (_, i) => ({
              role: "r"+i,
              sign: (i - (ribbonCount - 1)/2) / Math.max(1, ribbonCount - 1),
        ...
        const driftY = Math.sin(now * 0.00004 + 1.2) * 0.10; // -0.10..0.10 of height
        const curvePulse = 0.6 + 0.4 * Math.sin(now * 0.00020);
        ...
        for (let r = 0; r < specs.length; r++) {
          ...
          for (let i = 0; i <= segs; i++) {
            ...
            const centerCurve = 0.5 + Math.sin(nx * Math.PI * baseFreq + t * 7) * baseAmp + driftY;

            const weave = Math.sin(nx * Math.PI * (curveFreq + 0.5) + t * 13 + S.phase) * weaveAmp * (0.7 + 0.6 * weavePulse);
            const warp  = (n(nx * 1.8 + r * 0.7 + t * 2.5, t * 3.5 + r) - 0.5) * (noiseWarp * 1.4);

            const sepDrift = 0.10 * Math.sin(t * 8 + nx * 4.0 + r);
          ...
          for (let i = 1; i < pts.length; i++) {
            ...
            while (d <= segLen) {
              ...
              const hueTeal = 150, hueBlue = 210, huePurple = 285, hueRed = 350;
              const sweep = 0.5 + 0.5 * Math.sin((cx / w) * Math.PI * 1.1 + t * 7 + r);
              const redPulse = Math.max(0, Math.sin(t * 5 + r) - 0.94) / 0.06;
              const hue = lerpHue(lerpHue(hueTeal, hueBlue, sweep), huePurple, sweep * 0.6);
              ...
              const targetReduce = isLight ? Math.min(0.35, safeBandReduce) : safeBandReduce;
              const reduction = falloff * targetReduce;
              const alphaMul = Math.max(0, Math.min(1, 1 - reduction)) * S.alpha;
              ...
              grad.addColorStop(0.00, `hsla(${hueWithRed}, ${sat}%, ${lCore}%, ${baseAlpha * (isLight ? 0.95 : 0.80) * alphaMul})`);
              grad.addColorStop(0.45, `hsla(${hueWithRed + 18}, ${Math.min(100, sat * 0.9)}%, ${lMid}%, ${baseAlpha * (isLight ? 0.75 : 0.60) * alphaMul})`);
              grad.addColorStop(1.00, `hsla(${hueWithRed + 90}, 60%, ${lMid + 2}%, 0)`);

              aurCtx.shadowBlur = 12 + bloomAmt * 4;
              aurCtx.shadowColor = `hsla(${hueBlue}, 90%, ${isLight ? 88 : 78}%, ${baseAlpha * alphaMul})`;
        ...
        aurCtx.globalCompositeOperation = isLight ? "screen" : "lighter";
        aurCtx.globalAlpha = Math.min(isLight ? 0.55 : 0.60, eff);
        aurCtx.filter = `blur(${(isLight ? 8 : 10) + bloomAmt * (isLight ? 6 : 8)}px)`;
    ...
    theme, showInLight, lightSky,
    stars, starsInLight, starDensity, starSize, starTwinkle,
    intensity, maxOpacity, saturation, bloom,
    lightIntensity, lightMaxOpacity, lightSaturation, lightBloom,
    ribbonCount, speed, curveAmp, curveFreq, weaveAmp, noiseWarp,
    puffRadius, puffStepPx, safeBandTop, safeBandBottom, safeBandReduce
  ]);
